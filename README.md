...
StudyFlow 是一个用 Go 1.22+ 编写的学习成长后端。它把目标、学习任务、待办清单、智能日历、知识花园、单词学习、番茄专注和间隔复习放进同一个领域模型，并通过事件流把数据实时推送给客户端。核心服务使用 Go 标准库，并通过 MIT 许可的 `lunar-go` 提供可靠的中国农历与节气计算；数据会周期性持久化为 JSON 快照。  # noqa: E999
