...
- “每日一签”现为签筒抽签：点击摇签 → 出签 → 点击揭签，展示签号、原创生活签文与行动启发；首版 31 支签由服务端随机抽取，按中国标准时间每账号每天保留首次结果，刷新、并发点击和重试不会换签（不同日期可能抽到相同签）。支持往日已抽记录、收藏、感受、纸色与 PNG 分享，不预测运势。旧版已保存日签保留，当天已有记录时直接恢复；未抽的往日不能补抽。每次首次抽签也计入探索记录 500 条上限，复用快照和个人导出。未保存感受仅在当前页面保留；更新后需重启 Go 服务。
