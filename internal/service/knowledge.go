...
func (s *Service) DeleteKnowledgeNote(ctx context.Context, userID, noteID string) error {
	...
	if err := s.repo.DeleteKnowledgeNote(ctx, noteID); err != nil {
		return err
	}
...
func (s *Service) KnowledgeNote(ctx context.Context, userID, noteID string) (domain.KnowledgeNoteDetail, error) {
	...
	for _, edge := range edges {
		if edge.SourceID == note.ID {
			if edge.Resolved {
				detail.OutgoingLinks = append(detail.OutgoingLinks, edge)
			} else {
				detail.UnresolvedLinks = append(detail.UnresolvedLinks, edge)
			}
		}
		if edge.Resolved && edge.TargetID == note.ID {
			detail.Backlinks = append(detail.Backlinks, edge)
		}
	}
	return detail, nil
}
...
func (s *Service) ListKnowledgeNotes(ctx context.Context, userID string, input ListKnowledgeNotesInput) ([]domain.KnowledgeNoteSummary, error) {
	...
	for _, note := range notes {
		...
		if tag != "" && !containsStringFold(note.Tags, tag) {
			continue
		}
	...
	sort.SliceStable(items, func(i, j int) bool {
		if items[i].note.Pinned != items[j].note.Pinned {
			return items[i].note.Pinned
		}
...
func validateKnowledgeNote(title, content string, tags []string) (string, string, []string, error) {
	...
	if title == "" || utf8.RuneCountInString(title) > 160 {
		return "", "", nil, fmt.Errorf("%w: title is required and must not exceed 160 characters", domain.ErrInvalidInput)
	}
	...
	tags = cleanTags(tags)
	if len(tags) > 12 {
		return "", "", nil, fmt.Errorf("%w: at most 12 tags are allowed", domain.ErrInvalidInput)
	}
	for _, tag := range tags {
		if utf8.RuneCountInString(tag) > 48 {
			return "", "", nil, fmt.Errorf("%w: tags must not exceed 48 characters", domain.ErrInvalidInput)
		}
	}
...
func (s *Service) ensureKnowledgeTitleUnique(ctx context.Context, userID, exceptID, title string) error {
	...
	for _, note := range notes {
		if note.ID != exceptID && strings.EqualFold(strings.TrimSpace(note.Title), title) {
			return fmt.Errorf("%w: a note with this title already exists", domain.ErrConflict)
		}
	}
	return nil
}
...
func buildKnowledgeLinks(notes []domain.KnowledgeNote) ([]domain.KnowledgeLink, map[string][2]int) {
	...
	for _, note := range notes {
		byTitle[strings.ToLower(strings.TrimSpace(note.Title))] = note
	}
