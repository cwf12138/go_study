...
func (s *Server) createKnowledgeNote(w http.ResponseWriter, r *http.Request) {
	var body struct { Title string `json:"title"`; Content string `json:"content"`; Tags []string `json:"tags"`; Pinned bool `json:"pinned"` }
	if err := decodeJSON(w, r, &body); err != nil { writeError(w, invalidJSON(err)); return }
...
func (s *Server) listKnowledgeNotes(w http.ResponseWriter, r *http.Request) {
	...
	if value := r.URL.Query().Get("pinned"); value != "" {
		...
		if err != nil { writeError(w, fmt.Errorf("%w: pinned must be true or false", domain.ErrInvalidInput)); return }
		pinned = &parsed
	}
...
func (s *Server) deleteKnowledgeNote(w http.ResponseWriter, r *http.Request) {
	if err := s.service.DeleteKnowledgeNote(r.Context(), claimsFromContext(r.Context()).Subject, r.PathValue("note_id")); err != nil { writeError(w, err); return }
