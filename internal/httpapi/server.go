...
func NewHandler(svc *service.Service, tokens *security.TokenManager, bus *event.Bus, logger *slog.Logger) http.Handler {
	...
	private.HandleFunc("POST /api/v1/memo-folders", s.createMemoFolder)
	private.HandleFunc("GET /api/v1/memo-folders", s.listMemoFolders)
	private.HandleFunc("PATCH /api/v1/memo-folders/{folder_id}", s.updateMemoFolder)
	private.HandleFunc("DELETE /api/v1/memo-folders/{folder_id}", s.deleteMemoFolder)
	private.HandleFunc("POST /api/v1/memos", s.createMemoNote)
	private.HandleFunc("GET /api/v1/memos", s.listMemoNotes)
	private.HandleFunc("GET /api/v1/memos/overview", s.memoOverview)
	private.HandleFunc("GET /api/v1/memos/{note_id}", s.memoNote)
	private.HandleFunc("PATCH /api/v1/memos/{note_id}", s.updateMemoNote)
	private.HandleFunc("DELETE /api/v1/memos/{note_id}", s.trashMemoNote)
	private.HandleFunc("POST /api/v1/memos/{note_id}/restore", s.restoreMemoNote)
	private.HandleFunc("POST /api/v1/memos/{note_id}/duplicate", s.duplicateMemoNote)
	private.HandleFunc("DELETE /api/v1/memos/{note_id}/permanent", s.deleteMemoNotePermanently)
	private.HandleFunc("GET /api/v1/english/articles", s.englishArticles)
	private.HandleFunc("GET /api/v1/english/library", s.listEnglishReadings)
	private.HandleFunc("POST /api/v1/english/library", s.saveEnglishReading)
	private.HandleFunc("PATCH /api/v1/english/library/{reading_id}", s.updateEnglishReading)
	private.HandleFunc("DELETE /api/v1/english/library/{reading_id}", s.deleteEnglishReading)
	private.HandleFunc("GET /api/v1/english/overview", s.englishOverview)
	private.HandleFunc("GET /api/v1/literature/catalog", s.searchEBooks)
	private.HandleFunc("GET /api/v1/literature/shelf", s.listEBookShelf)
	private.HandleFunc("POST /api/v1/literature/shelf", s.addEBookToShelf)
	private.HandleFunc("GET /api/v1/literature/shelf/{reading_id}/content", s.ebookContent)
	private.HandleFunc("PATCH /api/v1/literature/shelf/{reading_id}/progress", s.updateEBookProgress)
	...
	private.HandleFunc("GET /api/v1/literature/classics/{work_id}", s.classicalWork)
	private.HandleFunc("GET /api/v1/literature/classic-studies", s.listClassicalStudies)
	private.HandleFunc("PUT /api/v1/literature/classic-studies/{work_id}", s.updateClassicalStudy)
	private.HandleFunc("GET /api/v1/literature/overview", s.literatureOverview)
